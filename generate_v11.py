# Python script to write build_toughbook_iso.sh for V11 (Vivaldi Only)
import os

script_content = r'''#!/bin/bash
# Revenant OS - Custom Agentic Linux ISO Builder for Panasonic Toughbook
# Version 11 - Definitive Installation & Boot Fix (Vivaldi Only)
...
ISO_NAME="revenant_os_toughbook_v11.iso"
...
# Install Vivaldi Browser exclusively (Resumable download loop)
echo "[*] Setting up Vivaldi repository and package..."
...
# Download Vivaldi deb with automatic resume (-C -) and retry
echo "[*] Downloading Vivaldi debian package directly..."
for i in {1..10}; do
  if curl -C - --retry 5 --retry-delay 5 -fsSL "https://downloads.vivaldi.com/stable/vivaldi-stable_8.2.4133.47-1_amd64.deb" -o /tmp/vivaldi.deb; then
    break
  fi
  echo "Vivaldi download interrupted. Resuming in 5s (attempt $i/10)..."
  sleep 5
done

# Install the downloaded package and all its dependencies
apt-get update
apt-get install -y /tmp/vivaldi.deb
...
echo "[*] Preparing ISO bootloader files..."
...
echo "[*] Packaging SquashFS (including /boot for native installer)..."
umount "$CHROOT_DIR/sys" 2>/dev/null || true
umount "$CHROOT_DIR/proc" 2>/dev/null || true
umount "$CHROOT_DIR/dev" 2>/dev/null || true

# Note: We do NOT exclude boot, so the installed system retains the native kernel!
mksquashfs "$CHROOT_DIR" "$IMAGE_DIR/live/filesystem.squashfs" -comp xz

echo "[*] Building ISO..."
# Run grub-mkrescue on host (WSL) using product flags to prevent Kali branding and build proper hybrid MBR
grub-mkrescue -o "$SCRIPT_DIR/$ISO_NAME" "$IMAGE_DIR" --product-name="Revenant OS" --product-version="V11"
...
print('Generated build_toughbook_iso.sh for V11 (Vivaldi Only) successfully')
