...
# Version 11 - Definitive Installation & Boot Fix
...
# Install Browser (Vivaldi with retry loop and fallback to Chromium)
...
VIVALDI_OK=false
for i in {1..5}; do
  if apt-get install -y --fix-missing vivaldi-stable; then
    VIVALDI_OK=true
    break
  fi
  echo "Vivaldi download attempt $i failed. Retrying in 10s..."
  sleep 10
  apt-get update || true
done

if [ "$VIVALDI_OK" = false ]; then
  echo "Vivaldi repo unavailable. Installing Chromium browser..."
