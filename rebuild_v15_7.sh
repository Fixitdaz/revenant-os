...
WORKSPACE_DIR="/var/tmp/toughbook_rebuild_v15_7"
PATCH_ROOT="/var/tmp/patch_root_v15_7"
ISO_SOURCE="$SCRIPT_DIR/revenant_os_toughbook_v15_5.iso"
ISO_TARGET="$SCRIPT_DIR/revenant_os_toughbook_v15_7.iso"
ISO_ALIAS="$SCRIPT_DIR/revenant_os_toughbook_V15.7.iso"
...
echo "=== Revenant OS V15.7 Installation Started ===" > "$LOG"
date >> "$LOG"
...
menuentry "Revenant OS V15.7 - Agentic Linux" --class debian --class gnu-linux --class gnu --class os {
    insmod gzio
    insmod part_msdos
    insmod ext2
    search --no-floppy --fs-uuid --set=root $UUID
...
menuentry "Revenant OS V15.7 (Recovery Mode)" --class debian --class gnu-linux --class gnu --class os {
    insmod gzio
    insmod part_msdos
    insmod ext2
    search --no-floppy --fs-uuid --set=root $UUID
...
) | zenity --progress --title="Installing Revenant OS V15.7" --text="Starting installation..." --percentage=0 --auto-close

if [ -f "$LOG" ] && grep -iq "Installing for i386-pc platform" "$LOG"; then
  zenity --info --title="Success" \
    --text="<b>Revenant OS V15.7 has been successfully installed to $DRIVE!</b>\n\nYou can now reboot and remove the USB drive."
else
...
menuentry "Revenant OS V15.7 - Agentic Core (Offline Local LLM)" {
    linux /live/vmlinuz boot=live components quiet splash
    initrd /live/initrd.img
}

menuentry "Revenant OS V15.7 (Safe Graphics / Failsafe)" {
...
echo "[*] Building V15.7 ISO with hybrid bootloader..."
grub-mkrescue -o "$ISO_TARGET" "$WORKSPACE_DIR/image" --product-name="Revenant OS" --product-version="V15.7"
cp -f "$ISO_TARGET" "$ISO_ALIAS"

echo "[*] Cleaning up workspace..."
rm -rf "$WORKSPACE_DIR" "$PATCH_ROOT"

echo "[*] Build Complete! Revenant OS V15.7 ISO ready at: $ISO_TARGET"
