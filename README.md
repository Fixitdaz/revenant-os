...
- **20,480 Token Context Window**: Full 20K context length with pristine `f16` Key-Value attention cache (eliminates 4-bit attention matrix distortion and stutter loops).
...
- **Hermes Agent**: Fully pre-configured with local custom provider (`name: "local"`), 20K context length, and auxiliary context compression. Run `hermes` in any terminal for autonomous multi-step reasoning.
...
- [**02: Agentic AI Core & Local LLM Manual**](./docs/02-AGENT-STACK.md) - llama-server, Qwen2.5 local model (20K context), OpenViking memory daemon, and agent orchestration.
